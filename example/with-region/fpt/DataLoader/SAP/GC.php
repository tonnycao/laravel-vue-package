...
class GC extends Loader
{
    ...
    public function __construct($job, $db = false)
    {
        ...
        $this->db = $db;
        $this->tag = $this->tag();
        $this->path = sprintf(config('dataloader.path'), $this->rt, $this->region);
    ...
    private function loadDataForType($folder = false)
    {
        ...
        foreach ($files as $loading_file) {
            ...
            if (stripos(strtolower($file_name),
                    config(sprintf('dataloader.SAP.%s.%s', $this->region, $this->type))) === false
            ) {
                continue;
            }
